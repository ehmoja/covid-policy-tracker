...
from app.log import action_log_callback
from app.log.action_log_model import ActionLogParams
...
def _build_metrics(func_name: str,
    ...
    kwargs_repr = {kw: repr(arg) for kw, arg in kwargs.items()}

    metrics = {
        'command': kwargs.get('command', func_name),
        'start_epoch_ms': get_epoch_millisec(),
        'host_name': socket.gethostname(),
        'pos_args_json': json.dumps(args_repr),
        'keyword_args_json': json.dumps(kwargs_repr),
    }  # type: Dict[str, Any]
