...
class UserSchema(AttrsSchema):
    ...
    def preprocess_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._str_no_value(data.get('linkedin_url')):
            data['linkedin_url'] = ''
            if data.get('GET_PROFILE_URL'):
                data['linkedin_url'] = data.get('GET_PROFILE_URL')(data['user_id'])  # type: ignore
...
def load_user(user_data: Dict) -> User:
    try:
        ...
        # Add linkedin_url from optional 'GET_PROFILE_URL' configuration method.
        # This methods currently exists for the case where the 'linkedin_url' is not included
        # in the user metadata.
        if _str_no_value(user_data.get('linkedin_url')) and app.config['GET_PROFILE_URL']:
            user_data['linkedin_url'] = app.config['GET_PROFILE_URL'](user_data['user_id'])
        # Add role to role_name
        if _str_no_value(user_data.get('role_name')):
            user_data['role_name'] = user_data.get('role')
